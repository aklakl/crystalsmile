...
def get_dentists_by_zipcode(zipcode):
    ...
    while True:
        ...
        try:
            ...
            logging.error(f"Error in ZIP code {zipcode}: {str(e)}")
            print(f"❌ Error in ZIP code {zipcode}: {str(e)}")
            break
...
async def fetch_rendered_html(url, timeout=30000):
    async with async_playwright() as p:
        ...
        try:
            ...
            logging.warning(f"Error fetching rendered HTML for {url}: {str(e)}")
            return None
        finally:
            await browser.close()

# Example of how you would integrate this into your script:
# This is a conceptual example. You will need to adapt it to your existing code.
# If you're currently fetching a URL like this:
#   import requests
...
#   some_url = "your_target_url_here"
#   rendered_html_content = await fetch_rendered_html(some_url)
#   if rendered_html_content:
#       soup = BeautifulSoup(rendered_html_content, 'html.parser')
#       # Now your find_all_tables_test(soup) should work on the fully rendered content
#       # And save_soup_html_to_file(soup) will save the rendered HTML
#   else:
#       print("Failed to get rendered HTML.")


def find_all_tables_test(soup_obj):
    print("\n--- Test: Finding all table tags ---")
    tables = soup_obj.find_all('table')
    print(f"Found {len(tables)} table tags.")
    if tables:
        for i, table in enumerate(tables):
            print(f"\nTable {i+1}:")
            # You might want to print a snippet or the whole table HTML
            # print(table.prettify())
    else:
        print("No table tags found in the soup object.")
    print("--- End of Test ---")

def save_soup_html_to_file(soup_obj, filename_prefix="soup_output"):
    print(f"\n--- Test: Saving soup HTML to file ---")
    if not soup_obj:
        print("Error: No soup object provided.")
        return

    # Create a unique filename with timestamp
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    output_filename = f"{filename_prefix}_{timestamp}.html"

    # Define the directory where to save the file.
    # Assuming 'data' directory is relative to the current working directory of the script.
    # The `crawler-from-NPI.py` is in `scraper`, and `data` is sibling to `scraper`.
    output_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), '..', 'data','tmp')
    ...
    try:
        ...
        print(f"Successfully saved HTML content to: {full_path}")
    except Exception as e:
...
if all_dentists:
    ...
    # Asynchronous processing for missing data
    async def process_missing_data(df, df_missing, csv_filename, missing_csv):
        """
        Asynchronously fetches missing data, updates the DataFrame, and saves the results.
        """
        ...
        updated_count = 0
        for npi, additional_data in results:
            if additional_data:
                # Find the index of the row to update in the main DataFrame
                ...
                if not df_idx.empty:
                    ...
                    # 1. Use Authorized Official Name if found
                    if additional_data.get('auth_name'):
                        df.at[df_idx, 'First Name'] = additional_data['auth_name']
                        df.at[df_idx, 'Last Name'] = 'N/A' # Set Last Name to N/A
                        updated = True
                    # 2. Fallback to name from <title> tag
                    elif additional_data.get('first_name'):
                    ...
                    # 3. Use Authorized Official Phone if found
                    if additional_data.get('auth_phone'):
                        df.at[df_idx, 'Phone'] = additional_data['auth_phone']
                        updated = True
                    # 4. Fallback to other phone number
                    elif additional_data.get('phone'):
        ...
        if updated_count > 0:
            ...
            for idx, row in df.iterrows():
                ...
                if pd.isna(row['Phone']) or row['Phone'] == 'N/A' or row['Phone'] == '':
                    missing_fields_after.append('Phone')
                if pd.isna(row['First Name']) or row['First Name'] == '':
                    missing_fields_after.append('First Name')
                if pd.isna(row['Last Name']) or row['Last Name'] == '':
                    missing_fields_after.append('Last Name')
                ...
                if missing_fields_after:
                    missing_data_after.append({
                        ...
                        'Organization Name': row['Organization Name'],
                        'City': row['City'],
                        'Missing Fields': '; '.join(missing_fields_after)
                    })
            ...
            if missing_data_after:
                ...
                os.remove(missing_csv) # Clean up if no longer needed
    ...
    # Save missing data list and then process it
    if missing_data:
        ...
        asyncio.run(process_missing_data(df, df_missing, csv_filename, missing_csv))
